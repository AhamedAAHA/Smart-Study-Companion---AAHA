...
function LibraryContent() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      <p className="mt-1 text-slate-600">Saved cheat sheets, explanations, and notes</p>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-3">
        <ul className="space-y-2 lg:col-span-1">
          {filtered.length === 0 ? (
            <p className="text-sm text-slate-500">Save materials from document study tools.</p>
          ) : (
            filtered.map((item) => (
              <li key={item._id}>
                <button
